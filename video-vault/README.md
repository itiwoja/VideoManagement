...
- 動画カードクリック → アプリ内プレイヤーで再生 (対応外サイトは「元サイトで開く」ボタン)、視聴回数 +1 + 履歴に1行記録
- 連続再生: プレイリスト詳細 / Vault 一覧から再生すると、その並び順を「再生キュー」として ◀▶ で前後移動、
  自動再生 ON なら動画終了時に次へ自動で進む (`<video>` 直リンク・抽出成功時のみ。YouTube 等の埋め込みは対象外)
