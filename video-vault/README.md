...
- **サムネイルのローカルキャッシュ**: 未実装 (Phase 4 候補、下記参照)。今は `thumbnail_url` を DB に文字列で持つのみなので、サムネ画像自体が消えると表示できなくなる。実装できれば「サムネ URL の寿命」からも独立し、3本柱が揃う

この3つ (削除への保険・リンク切れの可視化・将来のサムネ保全) をまとめて
